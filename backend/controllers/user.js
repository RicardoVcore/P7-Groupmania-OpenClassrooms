    ...
    bcrypt.hash(req.body.password, 8)
        .then(hash => {
            ...
            User.create({firstname, lastname, email, password})
                .then(newUser => {
                    console.log("new user signup", newUser.firstname, newUser.lastname, "userId:", newUser.id);
                    res.status(201).json()           
                })
...
exports.editUser = (req, res) => {  
    req.file ? req.body.profile = req.file.filename : console.log("same photo was saved"); 
    if (req.file) { 
        User.findOne({where: {id:req.params.id}})
            .then(user => {
                if(user.profile !== "defaultUserProfile.png") { 
                    ...
                    console.log("default image file cannot be deleted")
                }
            })
            .catch(error => res.status(400).json(error));
    }
